...
/*       struct menu *menu = sym_get_choice_menu(e->left.sym); */
/* 			struct menu *choice1; */
/*       struct expr *e; */
/*       char *or_delim = ""; */
/* 			// TODO: need to fix */
/*       /\* menu_for_each_sub_entry(menu, choice1) { // expr_list_for_each_sym(prop->expr, e, def_sym) { *\/ */
/* 			/\* 	struct symbol *def_sym = choice1->sym; *\/ */
/*       /\*   struct expr *e2; *\/ */
/*       /\*   char *and_delim = ""; *\/ */
/*       /\*   fprintf(out, "%s", or_delim); *\/ */
/*       /\*   or_delim = " || "; *\/ */
/* 			/\* 	struct menu *choice2; *\/ */
/*       /\*   menu_for_each_sub_entry(menu, choice2) { // expr_list_for_each_sym(prop->expr, e2, def_sym2) { *\/ */
/* 			/\* 		struct symbol *def_sym2 = choice2->sym; *\/ */
    ...
    for_all_symbols(sym) {
      ...
      if (sym_is_choice(sym)) {
        ...
        struct expr *e;
				...
				/* fprintf(stderr, "FJSDLK: %p\n", choice); */
				/* fprintf(stderr, "FJSDLK: %s\n", sym->name); */
        /* prop = sym_get_choice_prop(sym); */
				
				/* #define sym_is_optional(sym) ((sym)->flags & SYMBOL_OPTIONAL ? true : false) */

				/* Author: Masahiro Yamada <masahiroy@kernel.org>  2024-06-02 08:54:14 */
				/* Committer: Masahiro Yamada <masahiroy@kernel.org>  2024-07-15 12:08:36 */
				/* Parent: 8a22f867e330965539b9cb5ccc42c2b470330b43 (kconfig: turn defaults and additional prompt for choice members into error) */
				/* Child:  9b114520837a5f08b8eeeee30947bb9e7f44be1e (kconfig: remember the current choice while parsing the choice block) */
				/* Branches: master, remotes/linux-next/fs-current, remotes/linux-next/fs-next, remotes/linux-next/master, remotes/linux-next/pending-fixes, remotes/linux-next/stable, remotes/origin/for-next, remotes/origin/master */
				/* Follows: v6.10-rc7 */
				/* Precedes: v6.11-rc1 */
				...
				/* Author: Masahiro Yamada <masahiroy@kernel.org>  2024-04-22 12:41:04 */
				/* Committer: Masahiro Yamada <masahiroy@kernel.org>  2024-05-02 06:48:26 */
				/* Parent: 1da251c60def5df1475ed5e8670d7ba2b6a33983 (kconfig: remove SYMBOL_CHOICE flag) */
				/* Child:  a7c79cf3e4eb3c869148c81d2a7dc684bc8eeb07 (kconfig: remove SYMBOL_NO_WRITE flag) */
				/* Branches: master, remotes/linux-next/fs-current, remotes/linux-next/fs-next, remotes/linux-next/master, remotes/linux-next/pending-fixes, remotes/linux-next/stable, remotes/origin/arm64-uaccess, remotes/origin/for-next, remotes/origin/master */
				/* Follows: v6.9-rc5 */
				/* Precedes: v6.10-rc1 */

				/*     kconfig: remove 'optional' property support */

				/* Author: Kees Cook <kees@kernel.org>  2025-09-23 17:34:18 */
				/* Committer: Kees Cook <kees@kernel.org>  2025-09-24 17:23:35 */
				/* Parent: 0f57c75973bedc08a865b06ce1b73ae5b54477c0 (kconfig: introduce menu type enum) */
				/* Branches: master, remotes/linux-next/fs-current, remotes/linux-next/fs-next, remotes/linux-next/master, remotes/linux-next/pending-fixes, remotes/linux-next/stable, remotes/origin/master */
				/* Follows: v6.17-rc2 */
				/* Precedes: next-20250925, next-20250926, next-20250929, next-20251002, next-20251003, next-20251006, next-20251007, next-20251008, next-20251009, next-20251010, v6.18-rc1 */

				/*     kconfig: Add transitional symbol attribute for migration support */

				
				// print choice type, depending on config type and optional statement
				// optional apparently no longer in the grammar
				switch(sym->type) {
				case S_BOOLEAN:
      	...
      	struct menu *menu;
				struct menu *choice = list_first_entry(&sym->menus, struct menu, link);
				menu_for_each_sub_entry(menu, choice) {
					...
					if (sym) {
						/* fprintf(stderr, "%s\n", sym->name); */
						fprintf(output_fp, " %s%s", config_prefix, sym->name);  // any dependencies should be handled below with 'dep'
					} else {
						/* fprintf(stderr, "null sym"); */
						// NULL for menus, comments, and ifs (expr.h:244)
						// no need, since we are going over all syms anyway
					}
				}
        fprintf(output_fp, "|(");

				/* /\* expr_list_for_each_sym(choice, elem) { *\/ */
				/* fprintf(stderr, "FJSDLK: %p\n", choice); */
				/* menu_for_each_sub_entry(choice, elem) { // expr_list_for_each_sym(prop->expr, e, def_sym) { */
				/* 	fprintf(stderr, "FJSDLK\n"); */
        /*   /\* fprintf(output_fp, " %s%s", config_prefix, def_sym->name);  // any dependencies should be handled below with 'dep' *\/ */
        /*   fprintf(output_fp, " %s%s", config_prefix, elem->sym->name);  // any dependencies should be handled below with 'dep' */
        /* } */
				/* fprintf(stderr, "BOBOFJSDLK\n"); */
        /* fprintf(output_fp, "|("); */
