...
static PyObject * extract_kconfig(PyObject *self, PyObject *args) {
  ...
  const char *cargs[] = {
    "kconfig_extractor_v1", "--extract", "-o", outfile, "-e", arch, "-e", srcarch, "-e", "KERNELVERSION=kcu", "-e", "srctree=./", "-e", "CC=cc", "Kconfig"
  };
...
static PyMethodDef kconfig_extractor_v1_funcs[] = {
  {"extract_kconfig", (PyCFunction)extract_kconfig, METH_VARARGS, extract_kconfig_docs},
  {NULL}
};

void initkconfig_extractor_v1(void) {
  PyObject *m;
  
  m = Py_InitModule3("kconfig_extractor_v1", kconfig_extractor_v1_funcs,
                 "Extract Kconfig dependencies.");
  ExtractorError = PyErr_NewException("kconfig_extractor_v1.error", NULL, NULL);
