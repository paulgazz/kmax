...
# $KMAX_ROOT/analysis/schedule_cppstats.q"

if [[ "${version}" == "" || "${arch}" == "" ]]; then
    echo "USAGE: qsub -t 0-NJOBS -v version=version,arch=arch schedule_cppstats.q"
    exit 1
fi
...
python ${KMAX_ROOT}/analysis/collect_cppstats.py -j ${PBS_ARRAYID} ${version} ${arch}
