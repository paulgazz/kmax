...
compilation_units_worklist = os.path.join(kmaxdata.kmax_data, "configs_worklist_" + version + arch)
configs_datafile = kmaxdata.configs_datafile(version, arch)
  ...
  with lockfile.LockFile(compilation_units_worklist):
    with open(compilation_units_worklist, 'wb') as outf:
      with open(compilation_units_datafile, 'rb') as inf:
        for line in inf:
          outf.write(line)
  kmaxdata.backup_existing_file(configs_datafile)

superc_args = '-preprocessor -conditionConfigs'
...
while True:
  ...
  with tempfile.NamedTemporaryFile(dir=kmaxdata.kmax_scratch) as tmp:
    ...
    with lockfile.LockFile(configs_datafile):
      with open(configs_datafile, 'ab') as dataf:
