...
for version in versions:
  ...
  for arch in archs:
    if not instance.init_arch(arch):
      ...
      if do_kbuild:
        compunit_command = '/home/paul/research/repos/kmax/kbuildplus/linux_case_study.py'
        # if get_running_time:
        ...
        presence_conditions_datafile = kmaxdata.presence_conditions_datafile(version, arch)
        kmaxdata.backup_existing_file(presence_conditions_datafile)
        with lockfile.LockFile(presence_conditions_datafile):
          with open(presence_conditions_datafile, 'wb') as dataf:
            for presence_condition in buildsystemdata.compilation_units['presence_conditions']:
              dataf.write(presence_condition + '\n')
