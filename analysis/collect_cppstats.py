...
compilation_units_worklist = os.path.join(kmaxdata.kmax_data, "cppstats_worklist_" + version + arch)
...
while True:
  ...
  with tempfile.NamedTemporaryFile(dir=kmaxdata.kmax_scratch) as tmp:
    ...
    with lockfile.LockFile(cppstats_datafile):
      with open(cppstats_datafile, 'ab') as dataf:
