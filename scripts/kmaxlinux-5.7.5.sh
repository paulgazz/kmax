...
/usr/bin/time kmaxall -F .kmax/kmax.pending1 -DARCH=h8300 arch/h8300/Makefile > .kmax/kmax.pending2
/usr/bin/time kmaxall -F .kmax/kmax.pending2 -DARCH=mips arch/mips/Makefile > .kmax/kmax.pending3

# amd includes Makefiles explicitly instead of using Kbuild
/usr/bin/time kmaxall -F .kmax/kmax.pending3 -DFULL_AMD_DISPLAY_PATH=drivers/gpu/drm/amd/display -DFULL_AMD_PATH=drivers/gpu/drm/amd -DDISPLAY_FOLDER_NAME=display drivers/gpu/drm > .kmax/kmax.pending4
...
/usr/bin/time kmaxall -F .kmax/kmax.pending4 -DOS=Linux arch/um/ > .kmax/kmax.pending5
